...
color_temp() {
  local t=$1
  if (( $(echo "$t > 75" | bc -l) )); then echo -e "${RED}${t}°C${NC}"
  elif (( $(echo "$t > 60" | bc -l) )); then echo -e "${YELLOW}${t}°C${NC}"
...
color_usage() {
  local u=$1
  if (( $(echo "$u > 80" | bc -l) )); then echo -e "${RED}${u}%${NC}"
  elif (( $(echo "$u > 50" | bc -l) )); then echo -e "${YELLOW}${u}%${NC}"
  else echo -e "${GREEN}${u}%${NC}"
  fi
}
