...
# ─── colour helpers ──────────────────────────────────────────────────────────
RED=$'\e[0;31m'; YEL=$'\e[1;33m'; GRN=$'\e[0;32m'; NC=$'\e[0m'
pct(){ (( $(bc <<<"$1>80") ))&&printf "${RED}%.1f%%%s" "$1" "$NC"||
        (( $(bc <<<"$1>50") ))&&printf "${YEL}%.1f%%%s" "$1" "$NC"||
                                   printf "${GRN}%.1f%%%s" "$1" "$NC"; }
tmp(){ (( $(bc <<<"$1>75") ))&&printf "${RED}%.1f°C%s" "$1" "$NC"||
...
get_cpu_freq(){ sum=0 cnt=0
  for f in /sys/devices/system/cpu/cpu*/cpufreq/scaling_cur_freq; do
    [[ -r $f ]]&&sum=$((sum+$(<"$f")))&&((cnt++))
  done; ((cnt))&&echo $((sum/cnt/1000))||echo 0; }
get_cpu_temp(){ sensors -u 2>/dev/null | awk '/temp1_input/ {printf "%.1f",$2; exit}'; }
...
chrome_mem(){ kb=0; for p in $C_PIDS; do
  ...
  else kb=$((kb+$(awk '/VmRSS:/ {print $2}' /proc/$p/status)));fi;done;echo $((kb/1024));}
...
gpu_stats() {
  ...
  [[ -z $busy ]] && busy=$(jq -r '.engines."Render/3D".busy' <<<"$json")
...
# ─── accumulators ───────────────────────────────────────────────────────────
declare -A sum; for k in cu su cf ct gu gf gt cm cmp sm smp fps;do sum[$k]=0;done
cnt=0 start=$(date +%s)
...
while kill -0 "$ROOT" 2>/dev/null; do
  ...
  sys_used=$(((MEM_TOTAL-mem_free)/1024)); sys_tot=$((MEM_TOTAL/1024))
  ...
  printf "MEM : Chromium:%4d MB (%7s) | Sys:%4d/%4d MB (%7s)\n" \
         "$ch_mem" "$(pct "$ch_pct")" "$sys_used" "$sys_tot" "$(pct "$sys_pct")"
  printf "GPU : %7s @%4d MHz | FPS:%s\n" "$(pct "$gpu_busy")" "$gpu_freq" "$fps"

  # accum
  sum[cu]=$(bc -l <<<"${sum[cu]}+$ch_cpu"); sum[su]=$(bc -l <<<"${sum[su]}+$sys_cpu")
  sum[cf]=$((sum[cf]+cpu_freq));            sum[ct]=$(bc -l <<<"${sum[ct]}+$cpu_temp")
  sum[gu]=$(bc -l <<<"${sum[gu]}+$gpu_busy"); sum[gf]=$((sum[gf]+gpu_freq))
...
[[ $cnt -eq 0 ]] && { echo "No samples"; exit 1; }

avg(){ printf "%.1f" "$(bc -l <<<"${sum[$1]}/$cnt")"; } ; avg_i(){ echo $((sum[$1]/cnt)); }
