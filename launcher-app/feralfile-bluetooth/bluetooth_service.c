...
static void agent_method_call(GDBusConnection *conn,
                              ...
                              gpointer user_data) {
    log_info("[%s] Agent method called: %s on object %s", LOG_TAG, method_name, object_path);
    if (g_strcmp0(method_name, "Release") == 0) {
        log_info("[%s] Received BT Release", LOG_TAG);
        g_dbus_method_invocation_return_value(invocation, NULL);
    } else {
        GVariant *child = g_variant_get_child_value(parameters, 0);
        const gchar *device = child ? g_variant_get_string(child, NULL) : "unknown";

        if (g_strcmp0(method_name, "RequestPinCode") == 0) {
            log_info("[%s] Received BT RequestPinCode for device %s", LOG_TAG, device);
            ...
            log_info("[%s] Received BT RequestPasskey for device %s", LOG_TAG, device);
            g_dbus_method_invocation_return_dbus_error(
                invocation,
                "org.bluez.Error.Rejected",
            ...
            log_info("[%s] Received BT RequestConfirmation for device %s", LOG_TAG, device);
            g_dbus_method_invocation_return_value(invocation, NULL);
        } else if (g_strcmp0(method_name, "AuthorizeService") == 0) {
            log_info("[%s] Received BT AuthorizeService for device %s", LOG_TAG, device);
            g_dbus_method_invocation_return_value(invocation, NULL);
        } else if (g_strcmp0(method_name, "RequestAuthorization") == 0) {
            log_info("[%s] Received BT RequestAuthorization for device %s", LOG_TAG, device);
            g_dbus_method_invocation_return_value(invocation, NULL);
        } else if (g_strcmp0(method_name, "Cancel") == 0) {
            log_info("[%s] Received BT Cancel for device %s", LOG_TAG, device);
            g_dbus_method_invocation_return_value(invocation, NULL);
        } else if (g_strcmp0(method_name, "DisplayPinCode") == 0 || 
                   g_strcmp0(method_name, "DisplayPasskey") == 0) {
            log_info("[%s] Received BT DisplayPinCode/DisplayPasskey for device %s", LOG_TAG, device);
            g_dbus_method_invocation_return_value(invocation, NULL);
        } else {
            log_info("[%s] Received BT Not Supported for device %s", LOG_TAG, device);
            g_dbus_method_invocation_return_dbus_error(
                invocation,
...
/* ---------------- Service and Characteristic Property Getters ---------------- */
...
/* ---------------- Write Handlers for Characteristics ---------------- */
static void handle_write_value(GDBusConnection *conn,
                               ...
                               GDBusMethodInvocation *invocation,
                               gpointer user_data) {
    GVariant *array_variant = NULL, *options_variant = NULL;
...
/* ---------------- ObjectManager "GetManagedObjects" Handler ---------------- */
...
/* ---------------- D-Bus Objects VTable for ObjectManager ---------------- */
...
/* ---------------- Bluetooth Service Status Functions ---------------- */
...
static int wait_for_bluetooth_service() {
    ...
    log_error("[%s] Bluetooth service did not become active after %d attempts", LOG_TAG, MAX_RETRY_ATTEMPTS);
    return 0;
}
...
static void handle_property_change(GDBusConnection *connection,
    ...
    if (g_str_equal(iface, "org.bluez.Device1")) {
        GVariantIter iter;
        const gchar *key;
        GVariant *value;
    ...
    /* Do not unref changed_properties or invalidated_properties as they are borrowed */
}
...
static void* bluetooth_thread_func(void* arg) {
    ...
    while (retry_count < MAX_RETRY_ATTEMPTS) {
        ...
        if (!connection) {
            log_error("[%s] Failed to connect to D-Bus: %s", LOG_TAG, error ? error->message : "Unknown error");
            if (error) { g_error_free(error); error = NULL; }
            retry_count++;
            if (retry_count < MAX_RETRY_ATTEMPTS) {
                log_info("[%s] Retrying connection (attempt %d/%d)...", LOG_TAG, retry_count + 1, MAX_RETRY_ATTEMPTS);
                sleep(RETRY_DELAY_SECONDS);
                continue;
            }
            goto thread_exit;
        ...
        root_node = g_dbus_node_info_new_for_xml(
            "<node>"
        ...
        service_node = NULL;
        for (guint i = 0; root_node->nodes[i] != NULL; i++) {
            if (g_strcmp0(root_node->nodes[i]->path, "service0") == 0) {
                service_node = root_node->nodes[i];
                break;
            }
        }
        if (!service_node) {
        ...
        GDBusNodeInfo *setup_char_node = NULL;
        GDBusNodeInfo *cmd_char_node = NULL;
        GDBusNodeInfo *eng_char_node = NULL;
        for (guint i = 0; service_node->nodes[i] != NULL; i++) {
            if (g_strcmp0(service_node->nodes[i]->path, "setup_char") == 0)
                setup_char_node = service_node->nodes[i];
            else if (g_strcmp0(service_node->nodes[i]->path, "cmd_char") == 0)
                cmd_char_node = service_node->nodes[i];
            else if (g_strcmp0(service_node->nodes[i]->path, "eng_char") == 0)
                eng_char_node = service_node->nodes[i];
        }
        ...
        if (error || objects_reg_id == 0) {
            log_error("[%s] Failed to register ObjectManager interface: %s", LOG_TAG, error ? error->message : "Unknown error");
            if (error) { g_error_free(error); error = NULL; }
            goto thread_exit;
        }
...

        {
            GVariant *r = g_dbus_proxy_call_sync(
                agent_manager,
                "RegisterAgent",
                g_variant_new("(os)", "/org/bluez/agent", "NoInputNoOutput"),
        ...
        if (error || setup_char_reg_id == 0) {
            log_error("[%s] Failed to register setup characteristic object: %s", LOG_TAG, error ? error->message : "Unknown error");
            if (error) { g_error_free(error); error = NULL; }
            goto thread_exit;
        }
        ...
        if (error || cmd_char_reg_id == 0) {
            log_error("[%s] Failed to register command characteristic object: %s", LOG_TAG, error ? error->message : "Unknown error");
            if (error) { g_error_free(error); error = NULL; }
            goto thread_exit;
        }
        ...
        if (error || eng_char_reg_id == 0) {
            log_error("[%s] Failed to register engineering characteristic object: %s", LOG_TAG, error ? error->message : "Unknown error");
            if (error) { g_error_free(error); error = NULL; }
            goto thread_exit;
        }
...

        {
            ...
            if (error) {
                log_error("[%s] RegisterApplication failed: %s", LOG_TAG, error->message);
                g_error_free(error); error = NULL;
                goto thread_exit;
            }
            if (r)
        ...
        retry_count++;
        if (retry_count < MAX_RETRY_ATTEMPTS) {
            log_info("[%s] Attempting to reinitialize (attempt %d/%d)...", LOG_TAG, retry_count + 1, MAX_RETRY_ATTEMPTS);
            sleep(RETRY_DELAY_SECONDS);
        }
    }
thread_exit:
...
/* ---------------- Bluetooth Notify Function ---------------- */
void bluetooth_notify(const unsigned char* data, int length) {
    if (!connection) {
        log_error("[%s] Cannot notify: No active connection", LOG_TAG);
        return;
    }
    if (length <= 0 || data == NULL) {
        log_warning("[%s] No data to notify", LOG_TAG);
        return;
    }
...
/* ---------------- Bluetooth Get MAC Address Function ---------------- */
const char* bluetooth_get_mac_address() {
    static char mac_address[18] = {0};
    int dev_id = hci_get_route(NULL);
    int sock = hci_open_dev(dev_id);
    if (dev_id < 0 || sock < 0) {
        log_error("[%s] Could not get Bluetooth device info (dev_id=%d, sock=%d)", LOG_TAG, dev_id, sock);
        return NULL;
    }
    bdaddr_t bdaddr;
...
/* ---------------- Bluetooth Send Engineering Data Function ---------------- */
void bluetooth_send_engineering_data(const unsigned char* data, int length) {
    ...
    if (length <= 0 || data == NULL) {
        log_warning("[%s] No engineering data to send", LOG_TAG);
        return;
    }
    char hex_string[length * 3 + 1];
    for (size_t i = 0; i < (size_t)length; i++) {
...
/* ---------------- Bluetooth Initialization and Start Functions ---------------- */
